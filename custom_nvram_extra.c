 ...
 *  SMLOBJ
 *******************************************************************************/ 
static kal_int8 modem_test_string[50]; 
                    ...
                    kal_uint8 source)
{
    ...
    //guxinting add by 2016-07-13     
    kal_uint8 hw_test_mcc_mnc[3] = {0x00,0x10,0x1F}; // 00101 case
    #if defined(__CF8090_LIBERIA_LOCK__)
    kal_uint8 mtn_novafone_mcc_mnc[3] = {0x61,0x80,0x4F};  //novafone : 618 04	
    kal_uint8 cmcc_mcc_mnc[3] = {0x46,0x00,0x0F};  //cmcc : 460 00	debug
    #endif
    //end by guxinting 
    ...
    *remain = meta->retry_count;
    ...
    for (idx = 0; idx < meta->num; idx++)
    {
        ...
        code_len = sml_GetCode(cat,
 	    ...
 	    //if (SML_SIZE_OF_CAT_N == code_len)
           //{
                if (kal_mem_cmp(code, hw_test_mcc_mnc, code_len)==0
                    #if defined(__CF8090_LIBERIA_LOCK__)
                    ||(kal_mem_cmp(code, mtn_novafone_mcc_mnc, code_len) == 0)
                    ||(kal_mem_cmp(code, cmcc_mcc_mnc, code_len) == 0)
                    #endif
                )
                {
                    return KAL_TRUE;
